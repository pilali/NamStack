...
void Doubler::Smoother::reset (double sampleRate, double timeSeconds)
{
    coeff = (float) (1.0 - std::exp (-1.0 / (timeSeconds * sampleRate)));
    current = target;
}

float Doubler::nextRandom() noexcept
{
    // xorshift32
    rngState ^= rngState << 13;
    rngState ^= rngState >> 17;
    rngState ^= rngState << 5;
    return (float) ((double) rngState / 2147483648.0 - 1.0);
}
...
void Doubler::prepare (double sampleRate, int maxBlockSize)
{
    ...
    windowSamples = std::max (16, (int) std::round (fs * 0.001 * windowMs));
    lpCoeff = 1.0f - std::exp (-2.0f * kPi * wetToneHz / (float) fs);
    lpCoeff = std::clamp (lpCoeff, 0.0f, 1.0f);
...
{
    enabled = isEnabled;
    timeMs = std::clamp (newTimeMs, 2.0f, maxTimeMs);
    detuneCents = std::clamp (newDetuneCents, 0.0f, 30.0f);
    humanize = std::clamp (newHumanize, 0.0f, 1.0f);
    width = std::clamp (newWidth, 0.0f, 1.0f);
    mix = std::clamp (newMix, 0.0f, 1.0f);

    // Constant-power dry/wet blend.
    const auto mixAngle = mix * kPi * 0.5f;
    dryGain.setTarget (std::cos (mixAngle));
    wetGain.setTarget (std::sin (mixAngle));

    // Voice panning: spread the two takes out to the sides as width grows.
    for (int v = 0; v < 2; ++v)
    {
        const auto pan = (v == 0 ? -1.0f : 1.0f) * width;
        const auto angle = (pan + 1.0f) * kPi * 0.25f;
...
void Doubler::updateVoiceDrift (Voice& voice)
{
    ...
    {
        voice.driftTarget = nextRandom();
        voice.driftCountdown = (int) (fs * (0.3 + 0.45 * (nextRandom() + 1.0)));
    }
...
void Doubler::process (float* left, float* right, int numSamples)
{
    if (! enabled)
        return;
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        for (auto& voice : voices)
        {
            ...
            const auto g1 = std::sin (kPi * (float) (voice.phase / window));
            const auto g2 = std::sin (kPi * (float) (phase2 / window));

            auto sample = g1 * readInterpolated (std::max (1.0, baseDelay + voice.phase))
                        + g2 * readInterpolated (std::max (1.0, baseDelay + phase2));
        ...
        const auto dg = dryGain.next();
        const auto wg = wetGain.next();
